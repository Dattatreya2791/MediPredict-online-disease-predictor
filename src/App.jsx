...
function App() {
  ...
  const fetchUserWithToken = useCallback(async (token) => {
    try {
       console.log("Sending token to /api/me:", token);
  ...
  useEffect(() => {
    ...
    if (storedUser?.token && typeof storedUser.token === 'string' && storedUser.token.length > 10) {
       console.log("Using token:", storedUser.token);
